...
def parse_start_message(raw_message: str, *, max_chunk_duration_millis: int) -> StartMessage:
    ...
    if encoding not in SUPPORTED_AUDIO_ENCODINGS:
        raise WebsocketProtocolError("audio_format.encoding must be pcm_s16le or pcm_f32le")
    ...
    return StartMessage(chunk_duration_millis=chunk_duration_millis, encoding=encoding)
